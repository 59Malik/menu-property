...
AddEventHandler('Property:Announce', function(type, reason)
	...
	if type == 1 then
		TriggerClientEvent('esx:showAdvancedNotification', -1, "Agence Immobilière", "Ouverture", "L'Agence imobilière~s~ est ~g~ouverte~s~ et prêt à vendre des propriétés.", "CHAR_MARTIN", 0)
	elseif type == 2 then
		TriggerClientEvent('esx:showAdvancedNotification', -1, "Agence Immobilière", "Fermeture", "L'Agence imobilière~s~ est ~r~fermé~s~ à très bientôt.", "CHAR_MARTIN", 0)
	elseif type == 3 then
