...
if Config.PropertySettings.EnableBanner then 
	...
	PropertyBuilderMenu = RageUI.CreateSubMenu(PropertyActionsMenu, false, "Caractéristiques")
else 
	PropertyActionsMenu = RageUI.CreateMenu("Dynasty 8", "Intéractions disponibles")
	PropertyGestionMenu = RageUI.CreateSubMenu(PropertyActionsMenu, false, "Intéractions disponibles")
	PropertyInfosMenu = RageUI.CreateSubMenu(PropertyGestionMenu, false, "Intéractions disponibles")
	PropertyBuilderMenu = RageUI.CreateSubMenu(PropertyActionsMenu, false, "Caractéristiques")
end
        ...
        CreateThread(function()
            while MenuIsOpen do
                RageUI.IsVisible(PropertyActionsMenu, function()
					RageUI.Button("~EX_R*~ Créer une propriété", false, {RightLabel = "→→→"}, true, {
						onSelected = function()
							createPropertyType = 1 
						end
					}, PropertyBuilderMenu)
					RageUI.Button("~EX_R*~ Créer un garage", false, {RightLabel = "→→→"}, true, {
						onSelected = function()
							createPropertyType = 2
						end
					}, PropertyBuilderMenu)
					RageUI.Button("~EX_R*~ Gestion des propriété(s)", false, {RightLabel = "→→→"}, true, {}, PropertyGestionMenu)
					RageUI.List("~EX_R*~ Annonce", Config.BuilderSettings.AnnounceList, Config.BuilderSettings.AnnounceList.Index, nil, {}, true, {
					...
					RageUI.Button("~EX_R*~ Facturer un joueur", false, {}, true, {
                        onSelected = function()
                            ...
                            if ClosestPlayer ~= -1 and distance <= 3.0 then
                                if AmountBill ~= nil and type(AmountBill) == 'number' then
                                    if ReasonBill ~= nil then
                                        ...
                                        ESX.ShowNotification("~r~Veuillez entrer un motif de facturation.")
                                    end
                                else
					...
					if #allProperties > 0 then 
						...
						for k,v in pairs (allProperties) do 
							if Config.BuilderSettings.PropertyList.Index == 1 then 
								RageUI.Button(v.propertyLabel, false, {RightLabel = "~b~Intéragir~s~ →"}, true, { 
									onSelected = function()
						...
						RageUI.Separator("~r~Aucune propriété(s) disponibles.")
					end
				end)
				RageUI.IsVisible(PropertyInfosMenu, function()
					...
					for k,v in pairs (allProperties) do 
						if PropertyID == v.propertyID then 
							RageUI.Separator("Nom de la propriété : ~y~"..v.propertyLabel)
							if v.ownerName ~= "-" then 
								RageUI.Separator("Nom du propriétaire : ~o~"..tostring(v.ownerName))
							end
							if v.propertyEntering ~= nil then
								if v.maxStorage ~= nil then 
									RageUI.Separator("Capacité de stockage : ~b~"..v.maxStorage.."KG")
								end
								RageUI.Separator("Intérieur : ~r~"..tostring(Config.PropertiesInteriors[v.propertyInteriors].PropertyName))
							end
							if v.propertyGarage ~= nil and v.propertyRented ~= nil then 
								if v.garageInteriors ~= nil then 
									RageUI.Separator("Capacité du garage : ~b~"..tostring(#Config.GarageInteriors[v.garageInteriors].AllowedPositions.." places"))
								end
							end
							RageUI.Button("Mettre un point GPS", false, {RightLabel = "→"}, true, {
								onSelected = function()
									SetNewWaypoint(v.propertyEntering.x, v.propertyEntering.y)
								end
							})
							if v.propertyOwner == "-" then 
								RageUI.List("Attribuer la propriété : ", Config.BuilderSettings.SellList, Config.BuilderSettings.SellList.Index, false, {}, true, {
									onListChange = function(Index)
				...
				RageUI.IsVisible(PropertyBuilderMenu, function()
					...
					if createPropertyType == 1 then 
						...
						RageUI.Button("Définir la position de l'entrée", false, {}, true, {
							onSelected = function()
								viewsMarkers(true)
								Config.BuilderSettings.PropertyInfos.EnteringPos = {x = math.round(pos.x, 2), y = math.round(pos.y, 2), z = math.round(pos.z, 2)}
							end
						})
						RageUI.List("Espace de stockage", Config.BuilderSettings.StorageList, Config.BuilderSettings.StorageList.Index, false, {}, true, {
							onListChange = function(Index)
						...
						RageUI.Checkbox("Ajouter un garage", false, Config.BuilderSettings.AddGarageCheckbox, {}, {
							onChecked = function()
								AddGarage = true
							end,
							onUnChecked = function()
					...
					if AddGarage or createPropertyType == 2 then 
						...
						RageUI.Button("Définir la position du garage", "Position à indiquer pour rentrer dans le garage", {}, true, {
							onSelected = function()
								viewsMarkers(true)
								Config.BuilderSettings.PropertyInfos.GaragePos = {x = math.round(pos.x, 2), y = math.round(pos.y, 2), z = math.round(pos.z, 2)}
							end
						})
						RageUI.Button("Définir la position du rangement", "Position à indiquer pour ranger un véhicule", {}, true, {
							onSelected = function()
								viewsMarkers(true)
					...
					if createPropertyType == 1 then 
						if Config.BuilderSettings.PropertyInfos["NameOfProperty"] ~= nil and Config.BuilderSettings.PropertyInfos.EnteringPos ~= nil then 
							RageUI.Button("Confirmer", false, {}, true, {
								onSelected = function()
							...
							RageUI.Button("Confirmer", false, {}, false, {})
						end
					else
...
if Config.PropertySettings.EnableBanner then 
	...
	OwnedPropertyInteractionsMenu = RageUI.CreateMenu('Dynasty 8', "Intéractions disponibles")
end
                ...
                RageUI.IsVisible(OwnedPropertyInteractionsMenu, function()
					for k,v in pairs (InfosOfProperty) do 
						if isOwned == true then 
							RageUI.Separator("↓ ~g~"..v.propertyLabel.."~s~ ↓")
							RageUI.Button("Adresse : ~b~"..GetStreetForCoords(v.propertyEntering.x, v.propertyEntering.y, v.propertyEntering.z), false, {}, true, {})
...
if Config.PropertySettings.EnableBanner then 
	...
	PropertyGarageMenu = RageUI.CreateMenu('Dynasty 8', "Intéractions disponibles")
end
            ...
            while MenuIsOpen do
                ...
                RageUI.IsVisible(PropertyGarageMenu, function()
					for k,v in pairs (myProperties) do 
						if isOwned == true then 
							RageUI.Separator("↓ ~g~"..v.propertyLabel.."~s~ ↓")
							RageUI.Button("Adresse : ~b~"..GetStreetForCoords(v.propertyGarage.x, v.propertyGarage.y, v.propertyGarage.z), false, {}, true, {})
...
if Config.PropertySettings.EnableBanner then 
	...
	OwnedPropertyGestionMenu = RageUI.CreateMenu('Dynasty 8', "Intéractions disponibles")
end
            ...
            while MenuIsOpen do
                ...
                RageUI.IsVisible(OwnedPropertyGestionMenu, function()
					for k,v in pairs(myProperties) do 
						...
						if #GetInvite > 0 then 
							RageUI.Separator("↓ ~g~Demande en cours..~s~ ↓")
...
if Config.PropertySettings.EnableBanner then 
	...
	OwnedPropertyGarageMenu = RageUI.CreateMenu('Dynasty 8', "Intéractions disponibles")
	OwnedPropertyVehiclesMenu = RageUI.CreateSubMenu(OwnedPropertyGarageMenu, 'Dynasty 8', "Intéractions disponibles")
	OwnedGarageVehiclesMenu = RageUI.CreateSubMenu(OwnedPropertyGarageMenu, 'Dynasty 8', "Intéractions disponibles")
end
							...
							for k,v in pairs(myProperties) do 
								for t,b in pairs (myOwnedVehicles) do 
									if #myGarageVehicles == #Config.GarageInteriors[myGarageInteriors].AllowedPositions then 
										RageUI.Button("Véhicule : ~b~"..GetVehicleLabel(b.vehicle.model).."~s~ - [~o~"..b.plate.."~s~]", false, {RightLabel = "~b~Attribuer~s~ →"}, false, {})
									else
										RageUI.Button("Véhicule : ~b~"..GetVehicleLabel(b.vehicle.model).."~s~ - [~o~"..b.plate.."~s~]", false, {RightLabel = "~b~Attribuer~s~ →"}, true, {
											onSelected = function()
												if not garageCooldown then 
													garageCooldown = true 
				...
				RageUI.IsVisible(OwnedGarageVehiclesMenu, function()
					if waitingForVehicles then 
						if #myGarageVehicles > 0 then 
							if #myGarageVehicles < #Config.GarageInteriors[myGarageInteriors].AllowedPositions then 
								RageUI.Separator("Place(s) disponibles : ~r~"..#myGarageVehicles.."~s~/~r~"..#Config.GarageInteriors[myGarageInteriors].AllowedPositions)
							else
							...
							for k,v in pairs(myProperties) do 
								for t,b in pairs (myGarageVehicles) do 
									...
									if v.propertyID == v.propertyID then 
										RageUI.Button("Véhicule : ~b~"..GetVehicleLabel(b.data_vehicle.model).."~s~ - [~o~"..b.plate.."~s~]", false, {RightLabel = "["..OwnedPrefix.."]"}, true, {
											onSelected = function()
												if b.stored == 1 then 
													if not garageCooldown then 
														garageCooldown = true 
...
if Config.PropertySettings.EnableBanner then 
	...
	OwnedPropertyStorageMenu = RageUI.CreateMenu('Dynasty 8', "Intéractions disponibles")
	OwnedPropertyStorageAddMenu = RageUI.CreateSubMenu(OwnedPropertyStorageMenu, 'Dynasty 8', "Intéractions disponibles")
	OwnedPropertyStorageRemoveMenu = RageUI.CreateSubMenu(OwnedPropertyStorageMenu, 'Dynasty 8', "Intéractions disponibles")
end
									...
									for t,b in pairs(v.dataMoney) do 
										if v.propertyID == propertyID then 
											if player ~= -1 and distance <= 1.2 then
												...
												if t == "black_money" then 
													RageUI.List("Argent sale : ~r~"..v.dataMoney["black_money"].count.."$", Config.BuilderSettings.dataMoneyList[1], Config.BuilderSettings.dataMoneyList[1].Index, false, {}, true, {
														onListChange = function(Index)
														...
														onSelected = function(Index)
															if Index == 1 then 
																if not actionCooldown then 
																	actionCooldown = true 
																	local count = Input("DepositBlackMoney", "Montant à déposer dans le stock", "", 6)
																	if count ~= nil then 
															...
															elseif Index == 2 then 
																if not actionCooldown then 
																	actionCooldown = true 
																	local count = Input("WithdrawBlackMoney", "Montant à retirer du stock", "", 6)
																	if count ~= nil then 
																		count = tonumber(count)
																		if type(count) == "number" then 
													...
													RageUI.List("Argent propre : ~g~"..v.dataMoney["cash"].count.."$", Config.BuilderSettings.dataMoneyList[2], Config.BuilderSettings.dataMoneyList[2].Index, false, {}, true, {
														onListChange = function(Index)
														...
														onSelected = function(Index)
															if Index == 1 then 
																if not actionCooldown then 
																	actionCooldown = true 
																	local count = Input("DepositMoney", "Montant à déposer dans le stock", "", 6)
																	if count ~= nil then 
																		count = tonumber(count)
																		if type(count) == "number" then 
															...
															elseif Index == 2 then 
																if not actionCooldown then 
																	actionCooldown = true 
																	local count = Input("WithdrawMoney", "Montant à retirer du stock", "", 6)
																	if count ~= nil then 
																		count = tonumber(count)
																		if type(count) == "number" then 
				...
				RageUI.IsVisible(OwnedPropertyStorageAddMenu, function()
					...
					if #ESX.PlayerData.inventory > 0 then 
						for k,v in pairs(ESX.PlayerData.inventory) do
							if v.count > 0 then 
								RageUI.Button(v.label.."~s~ - [~r~"..v.count.."~s~]", false , {RightLabel = "~b~Déposer~s~ →"}, true , {
									onSelected = function()
										if not actionCooldown then 
											actionCooldown = true 
						...
						RageUI.Separator("~r~Aucun item(s) sur vous.")
					end
				end)
				RageUI.IsVisible(OwnedPropertyStorageRemoveMenu, function()
					if WaitStorage == true then 
						RageUI.Separator("Capacité : ~r~"..countStorage.count.."~s~/~r~"..countStorage.max.."KG")
					end
					if countStorage.count > 0 then 
								...
								if v.data ~= nil then 
									for t,b in pairs(v.data) do 
										if v.data[t].count > 0 then 
											RageUI.Button(v.data[t].label.."~s~ - [~r~"..v.data[t].count.."~s~]", false , {RightLabel = "~b~Retirer~s~ →"}, true , {
												onSelected = function()
													if not actionCooldown then 
														actionCooldown = true 
						...
						WaitStorage = false
						RageUI.Separator("")
						RageUI.Separator("~r~Vous n'avez aucun item(s)")
						RageUI.Separator("")
					end
				end)
                Wait(1)
...
if Config.PropertySettings.EnableBanner then 
	...
	PropertyOwnerMenu = RageUI.CreateMenu('Dynasty 8', "Intéractions disponibles")
	PropertyOwnerAddMenu = RageUI.CreateSubMenu(PropertyOwnerMenu, "Dynasty 8", "Liste des joueurs")
	PropertyOwnerListMenu = RageUI.CreateSubMenu(PropertyOwnerMenu, "Dynasty 8", "Liste des co-propriétaires")
end
											...
											if user.name ~= v.playerName then 
												if v.playerName ~= nil then 
													if v.playerName == b.ownerName then 
														RageUI.Button(v.playerName, false, {RightLabel = "~b~Attribuer~s~ →"}, false, {})
													else
									...
									elseif #b.ownerList == 0 then 
										if v.playerName ~= nil then 
											if v.playerName == b.ownerName then 
												...
												RageUI.Button(v.playerName, false, {RightLabel = "~b~Attribuer~s~ →"}, true, {
													onSelected = function()
														if not ownerCooldown then 
															ownerCooldown = true 
															TriggerServerEvent("Property:addOwner", b.propertyID, v.id, b.ownerList)
				...
				RageUI.IsVisible(PropertyOwnerListMenu, function()
					for k,v in pairs(myProperties) do 
						if #v.ownerList > 0 then 
							...
							RageUI.Separator("~r~Aucun co-propriétaire(s).")
						end
					end
				end)
                Wait(1)
